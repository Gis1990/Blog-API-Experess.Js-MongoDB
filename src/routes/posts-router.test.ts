...
describe("endpoint /posts ", () => {
    ...
    it("3.Should return status 201 and correct new post (/post) ", async () => {
        ...
        await request(app).post("/posts").send(correctNewPost2).expect(401);
    ...
    it("4.Should return status 201  (/post) ", async () => {
        ...
        await request(app).post(`/posts/${postId}/comments`).send({ content: outputComment.content }).expect(401);
        ...
        await request(app).get(`/posts/8/comments`).expect(404);
        // Test successfully retrieving the comments for a post and expecting a status code of 200
        const response8 = await request(app).get(`/posts/${postId}/comments`).expect(200);
    ...
    it("6.Should return status 201 and correct new post (/get) ", async () => {
        ...
        const response2 = await request(app).get(`/posts/${postId}`).expect(200);
        ...
        await request(app).put(`/posts/5`).set("authorization", "Basic YWRtaW46cXdlcnR5").expect(404);
        // Test updating a post without being logged in and expecting a status code of 401
        await request(app).put(`/posts/${postId}`).send(correctDataForUpdating).expect(401);
        ...
        await request(app).delete(`/posts/${postId}`).set("authorization", "Basic YWRtaW46cXdlcnR5").expect(204);
        // Test trying to delete a non-existent post and expecting a status code of 404
        await request(app).delete(`/posts/${postId}`).set("authorization", "Basic YWRtaW46cXdlcnR5").expect(404);
    ...
    it("14.Should return status 204 and change like status (/put) ", async () => {
        ...
        await request(app).put(`/posts/${postId}/like-status`).send({ likeStatus: "Like" }).expect(401);
        ...
        const response6 = await request(app).get(`/posts/${postId}`).expect(200);
