...
const createPostForTestingInBlogs = (titleLen:number,shortDescriptionLen:number,contentLen:number,blogId:string) => {
    return{
...
describe('endpoint /blogs ',  () => {
    ...
    it('9.Should return status 201 and newly created blog1 (/post)+ Should return blog 1 status 200  (/get )'+
        ...
        expect(response5.body).toEqual(createDbReturnDataForAllBlogs(2,1,1,2,
        ...
        expect(response6.body).toEqual(createDbReturnDataForAllBlogs(2,2,1,2,
        ...
        await request(app)
            .put(`/blogs/${blog2?.id}`)
            .set('authorization', 'Basic YWRtaW46cXdlcnR5')
            .send({name: updatedCorrectBlog.name,description:updatedCorrectBlog.description, websiteUrl: updatedCorrectBlog.websiteUrl})
            .expect(204)
        ...
        const response9=await request(app)
            .put(`/blogs/${blog3?.id}`)
            .set('authorization', 'Basic YWRtaW46cXdlcnR5')
            .send({name: updatedIncorrectBlog.name,description:updatedIncorrectBlog.description, websiteUrl: updatedIncorrectBlog.websiteUrl})
            .expect(400)
        ...
        const response15=await request(app)
    ...
    it('12.Should return status 404 with incorrect Id (/put)', async () => {
        await request(app)
            .put(`/blogs/5`)
            .set('authorization', 'Basic YWRtaW46cXdlcnR5')
            .expect(404)
    ...
    it('13.Should return status 404 for non existing id (/delete)', async () => {
        await request(app)
            .delete(`/blogs/5`)
            .set('authorization', 'Basic YWRtaW46cXdlcnR5')
            .expect(404)
    ...
    it('14.Should return status 404 (/get) ', async () => {
        await request(app)
            .get(`/blogs/5/posts`)
            .expect(404)
    })
